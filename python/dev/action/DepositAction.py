...
class DepositAction(Action):
    ...
    def apply(self, agents):
        ...
        if(token_type == Deposit.TYPE_REBASE):
            token.add_event(Deposit(apy, token_delta, time_delta, address))
        elif(token_type == Deposit.TYPE_NONREBASE): 
            token.add_event(Deposit(0, token_delta, time_delta, address))
