    ...
    Service
{
  ...
  function getDelegateServiceRegistry() public virtual view returns (address delegateServiceRegistry) {
    delegateServiceRegistry = DelegateServiceLogic._getDelegateServiceRegistry();
  }

  function setDeploymentSalt(
    bytes32 deploymentSalt
  ) public virtual isNotImmutable(IService.setDeploymentSalt.selector) override(IDelegateService,Service) returns (bool success) {
    success = super.setDeploymentSalt(deploymentSalt);
  }

  function supportsInterface(bytes4 interfaceId) public view virtual override(IDelegateService,Service) returns (bool isSupported) {
    isSupported = super.supportsInterface(interfaceId);
  }

  function getCreate2Pedigree() public virtual view override(IDelegateService,Service) returns (Create2Pedigree memory pedigree) {
    pedigree = super.getCreate2Pedigree();
  }

  function getServiceDefs() public view virtual override(IDelegateService,Service) returns (ServiceDef[] memory serviceDef) {
