...
contract MinimalProxyGeneratorMock {
  ...
  function deployMinimalProxy(address target) external returns (address minimalProxy) {
    minimalProxy = CreateUtils._deploy(MinimalProxyGeneratorLogic._generateMinimalProxyInitCode(target));
  }
  ...
  function deployMinimalProxyWithSalt(address target, bytes32 salt) external returns (address minimalProxy) {
    minimalProxy = Create2Utils._deployWithSalt(MinimalProxyGeneratorLogic._generateMinimalProxyInitCode(target), salt);
  }
  ...
  function _calculateMinimalProxyDeploymentAddress(
    ...
    return Create2Utils._calculateDeploymentAddress(
      address(this),
      keccak256(MinimalProxyGeneratorLogic._generateMinimalProxyInitCode(target)),
      salt
    );
  }
