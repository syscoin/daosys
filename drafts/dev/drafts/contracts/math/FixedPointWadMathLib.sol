...
library FixedPointWadMathLib {
    ...
    function _mulWadUp(uint256 x, uint256 y) internal pure returns (uint256) {
        return _mulDivUp(x, y, WAD); // Equivalent to (x * y) / WAD rounded up.
    }
    ...
    function _divWadUp(uint256 x, uint256 y) internal pure returns (uint256) {
      // require( y != 0, "FixedPointWadMathLib:_divWadDown:: Attempting to divide by 0");
      return _mulDivUp(x, WAD, y); // Equivalent to (x * WAD) / y rounded up.
    }
