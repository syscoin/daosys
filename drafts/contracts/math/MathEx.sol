...
library MathEx {
     ...
     */
    function reducedFraction(Fraction memory fraction, uint256 max)
        internal
        pure
        returns (Fraction memory)
    {
        uint256 scale = Math.ceilDiv(Math.max(fraction.n, fraction.d), max);
        Fraction memory reduced = Fraction({
            n: fraction.n / scale,
            d: fraction.d / scale
        });
        if (reduced.d == 0) {
    ...
    function isInRange(
        ...
        Uint512 memory min = mul512(
            baseSample.n,
            offsetSample.d * (PPM_RESOLUTION - maxDeviationPPM)
        );
        Uint512 memory mid = mul512(
            baseSample.d,
            offsetSample.n * PPM_RESOLUTION
        );
        Uint512 memory max = mul512(
            baseSample.n,
            offsetSample.d * (PPM_RESOLUTION + maxDeviationPPM)
        );
    ...
    function sub256(Uint512 memory x, uint256 y)
        internal
        pure
    ...
    function sub512(Uint512 memory x, Uint512 memory y)
        internal
        pure
        returns (Uint512 memory ret)
    {
        unchecked {
            ret.lo = x.lo - y.lo;
            ret.hi = (x.hi - y.hi) - (x.lo < y.lo ? 1 : 0); 
        }
    }

    function add512(Uint512 memory x, Uint512 memory y)
        internal
        pure
        returns (Uint512 memory ret)
    {
        unchecked {
            ret.lo = x.lo + y.lo;
            ret.hi = (x.hi + y.hi) + (ret.lo < x.lo ? 1 : 0);
        }
    }
     ...
     */
    function gt512(Uint512 memory x, Uint512 memory y)
        internal
        pure
        returns (bool)
    {
     ...
     * @dev returns the value of `x < y`
     */
    function lt512(Uint512 memory x, Uint512 memory y)
        internal
        pure
        returns (bool)
    {
     ...
     * @dev returns the value of `x >= y`
     */
    function gte512(Uint512 memory x, Uint512 memory y)
        internal
        pure
        returns (bool)
    {
     ...
     * @dev returns the value of `x <= y`
     */
    function lte512(Uint512 memory x, Uint512 memory y)
        internal
        pure
        returns (bool)
    {
